...
export default async function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-surface px-4 py-6 text-[color:var(--text-main)] md:px-8">
      <div className="theme-card mx-auto flex min-h-[calc(100vh-3rem)] max-w-7xl flex-col rounded-[2rem] p-6 shadow-ambient backdrop-blur md:p-8">
        <header className="flex flex-col gap-6 border-b border-outline-soft/30 pb-8 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium theme-text-muted">نظام إنتاجية عربي مصمم بروح تحريرية حديثة</p>
          ...
          <div className="flex gap-3 self-start">
            ...
            {!session ? (
              <Link
                href="/login"
                className="theme-muted inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold transition hover:opacity-90"
              >
                تسجيل الدخول
              </Link>
            ) : null}
        ...
        <section className="grid flex-1 gap-6 pt-8 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-[2rem] bg-hero-grid p-8 shadow-ambient">
            <div className="max-w-2xl animate-fade-in-up">
              <span className="theme-card inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium">
              ...
              <p className="mt-6 max-w-xl text-lg leading-8 theme-text-muted">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-3">
              {features.map((feature) => (
                <div key={feature.title} className="theme-card rounded-3xl p-5 shadow-ambient">
                  <feature.icon className="h-5 w-5 text-primary" />
                  <h3 className="mt-4 text-lg font-semibold">{feature.title}</h3>
                  <p className="mt-2 text-sm leading-7 theme-text-muted">{feature.description}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <div className="theme-muted rounded-[2rem] p-6">
              <p className="text-sm font-medium theme-text-muted">الصفحات المتوفرة</p>
